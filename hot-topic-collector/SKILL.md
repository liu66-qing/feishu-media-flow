...
- LLM 不可用时使用本地关键词评分兜底
- LLM 或实时数据不足时，会用明确标记为 `fallback` 的选题补足结果
- 输出中区分 `live`、`fallback` 等数据状态，并记录 `degraded_platforms`
...
- `degraded_platforms`：发生降级的平台
- `cache_used`：是否使用 fallback/cache 类结果
...
当前版本已完成 Skill 独立运行接口、公开热点采集、LLM 筛选、本地兜底、平台降级标记和测试 fixture。

测试 fixture 已验证在 LLM 不可用、抖音和小红书公开接口失败时，仍可输出 8 条有效选题，并将降级平台写入 `degraded_platforms`。
