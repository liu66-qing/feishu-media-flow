...
def collect_topics(platforms: List[str]) -> tuple[List[Topic], List[str]]:
    all_topics: List[Topic] = []
    errors = []

    for platform in platforms:
        try:
            all_topics.extend(collect_platform_topics(platform))
    ...
    return dedupe_topics(all_topics), errors
...
def generate_result(input_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_topics, fetch_errors = collect_topics(platforms)
    llm_enabled = False
    llm_error = ""
