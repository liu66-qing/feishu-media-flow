...
## 2026 暑期补充范围：从文字画像升级为图文传播模型

任务书中的“三平台流量密码”按以下口径验收，不再等同于简单关键词抓取或文案模仿：

1. 每个平台目标为100条候选样本、30条机器短名单、10-15条人工批准核心样本；机器短名单不得直接污染正式画像。
2. 样本评分拆分为来源证据、准大一/AI受众匹配、相对互动、视觉完整度、校园场景可迁移性和异常惩罚。
3. 画像同时输出语言规律、封面规律、模板族、视觉隐喻、心理机制、证据样本ID、置信度和证据缺口。
4. 内容分为“AI时代个人价值/能力焦虑的招新情绪线”和“来自四条Line真实任务的技术分享线”；成果展示独立成栏。
5. 焦虑传播必须形成“时代变化→旧路线缺口→自我差距→可执行路径→社团入口→进群行动”，禁止虚构淘汰、羞辱零基础和承诺保研就业。
6. 图片模型只生成无文字手绘底图；HTML模板确定性叠加中文、Logo、平台比例和临时二维码。
7. 真实发布后以曝光、阅读/播放、收藏、评论、进群转化和人工修改率回流；没有后台数据时必须标记 unavailable/degraded。
